...
header('Content-Disposition: attachment;filename=printer_assets.csv');
...
// Query the database for printer assets
$query = "SELECT * FROM assets WHERE ast_category = 'printer' AND user_id = $user_id";
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    fputcsv($output, ['No assets found in the printer category']);
}
