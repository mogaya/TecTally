...
header('Content-Disposition: attachment;filename=peripherals_assets.csv');
...
// Query the database for Peripheral assets
$query = "SELECT * FROM assets WHERE ast_category = 'Peripheral' AND user_id = $user_id";
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    fputcsv($output, ['No assets found in the Peripheral category']);
}
